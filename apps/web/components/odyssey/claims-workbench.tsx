...
export default function ClaimsWorkbench() {
  ...
  return <div className={styles.claimsWorkbench}>
    ...
    <section className={styles.claimComposer} aria-labelledby="claim-composer-title"><div><p className={styles.eyebrow}>New claim</p><h2 id="claim-composer-title">Compose from evidence</h2><p>{evidence.length > 0 ? `${evidence.length} exact source unit${evidence.length === 1 ? "" : "s"} arrived from the reader.` : "Select lines in the reader, or record an unsupported question explicitly."}</p>{evidence.length === 0 && <Link href="/odyssey/read/1">Select text in the reader →</Link>}</div>
