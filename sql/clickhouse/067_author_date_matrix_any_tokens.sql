...
SELECT
    ...
    countIf(hasAnyTokens(lower(p.passage_text), {terms:Array(String)})) AS mention_count,
    countIf(o.observation_id != '') AS observation_count,
    groupUniqArrayIf(p.passage_id, hasAnyTokens(lower(p.passage_text), {terms:Array(String)})) AS passage_ids
FROM
(
