CREATE VIEW IF NOT EXISTS author_date_matrix AS
SELECT
    e.author_id,
    e.author_display_name,
    e.entry_date,
    countIf(hasToken(lower(p.passage_text), lower({term:String}))) AS mention_count,
    countIf(o.observation_id != '') AS observation_count,
    groupUniqArrayIf(p.passage_id, hasToken(lower(p.passage_text), lower({term:String}))) AS passage_ids
FROM
(
...
(
    ...
    WHERE canonical_term = {term:String}
