...
def test_empty_database_bootstraps_all_task_tables() -> None:
    ...
    assert [migration.version for migration in applied] == [
        f"{number:03}" for number in range(1, 8)
    ]
...
def test_bootstrap_is_idempotent() -> None:
    ...
    assert len(first) == 7
    assert second == ()
    assert len(client.migrations) == 7
...
def test_migration_files_are_single_idempotent_table_creates() -> None:
    migrations = load_migrations()

    assert len(migrations) == 7
    for migration in migrations:
        assert migration.sql.count(";") == 1
        assert "CREATE TABLE IF NOT EXISTS" in migration.sql
